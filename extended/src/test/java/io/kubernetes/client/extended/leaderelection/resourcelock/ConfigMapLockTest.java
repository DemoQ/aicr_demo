...
class ConfigMapLockTest {
  ...
  private static final String CONFIGMAP_PATH =
      "/api/v1/namespaces/" + NAMESPACE + "/configmaps";

  private ApiClient apiClient;
  private ConfigMapLock lock;
  ...
  void setup() {
    ...
    lock = new ConfigMapLock(NAMESPACE, NAME, IDENTITY, apiClient);
  }

  private V1ConfigMap configMap(LeaderElectionRecord record) {
    ...
    return new V1ConfigMap().metadata(metadata);
  }
  ...
  void getRecordFromAnnotation() throws ApiException {
    ...
    apiServer.stubFor(
        get(urlPathEqualTo(CONFIGMAP_PATH + "/" + NAME))
            .willReturn(
                aResponse()
                    .withStatus(200)
                    .withBody(apiClient.getJSON().serialize(configMap(expected)))));
    ...
    apiServer.stubFor(
        get(urlPathEqualTo(CONFIGMAP_PATH + "/" + NAME))
            .willReturn(
                aResponse()
                    .withStatus(200)
                    .withBody(apiClient.getJSON().serialize(configMap(null)))));
  ...
  void createConfigMap() {
    ...
    apiServer.stubFor(
        post(urlPathEqualTo(CONFIGMAP_PATH))
            .willReturn(
                aResponse()
                    .withStatus(201)
                    .withBody(apiClient.getJSON().serialize(configMap(record)))));

    assertThat(lock.create(record)).isTrue();
    apiServer.verify(1, postRequestedFor(urlPathEqualTo(CONFIGMAP_PATH)));
  }

  @Test
  void createConfigMapConflict() {
    apiServer.stubFor(post(urlPathEqualTo(CONFIGMAP_PATH)).willReturn(aResponse().withStatus(409)));
  ...
  @Test
  void updateConfigMapAfterGet() throws ApiException {
    LeaderElectionRecord record =
        new LeaderElectionRecord(IDENTITY, 10, new Date(1000L), new Date(2000L), 0);
    apiServer.stubFor(
        get(urlPathEqualTo(CONFIGMAP_PATH + "/" + NAME))
            .willReturn(
                aResponse()
                    ...
                    .withBody(apiClient.getJSON().serialize(configMap(record)))));
    apiServer.stubFor(
        put(urlPathEqualTo(CONFIGMAP_PATH + "/" + NAME))
            .willReturn(
                aResponse()
                    .withStatus(200)
                    .withBody(apiClient.getJSON().serialize(configMap(record)))));
    ...
    apiServer.verify(1, putRequestedFor(urlPathEqualTo(CONFIGMAP_PATH + "/" + NAME)));
  }

  @Test
  void updateConfigMapServerError() throws ApiException {
    LeaderElectionRecord record =
        new LeaderElectionRecord(IDENTITY, 10, new Date(1000L), new Date(2000L), 0);
    apiServer.stubFor(
        get(urlPathEqualTo(CONFIGMAP_PATH + "/" + NAME))
            .willReturn(
                aResponse()
                    ...
                    .withBody(apiClient.getJSON().serialize(configMap(record)))));
    apiServer.stubFor(
        put(urlPathEqualTo(CONFIGMAP_PATH + "/" + NAME)).willReturn(aResponse().withStatus(500)));
    lock.get();
